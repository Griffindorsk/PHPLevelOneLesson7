<?php
function product_generation() {//формирование записи из случайного набора данных
...
function put_in_basket($product_selected) {
    ...
    $product_in_basket = sql_request("SELECT `pr_number`,`quantity` FROM basket WHERE pr_number=\"$pr_number\"");
     if (gettype($product_in_basket) <> 'array') {
        $quantity = 1;
        //добавляем новый товар в корзину
        sql_request("INSERT INTO `basket` (`id_basket`, `id_customer`, `pr_number`, `pr_name`, `gender`, `size`, `color`, `photo`,`description`,`price_unit`, `quantity`) VALUES (NULL,'best_user','$pr_number','$pr_name','$gender','$size','$color','$photo', '$description_index','$price_unit','$quantity')");
    } else {
        $quantity = $product_in_basket[0]['quantity'] + 1;
        //обновляем количество ранее выбранного продукта
        sql_request("UPDATE `basket` SET `quantity`=$quantity WHERE pr_number=\"$pr_number\"");
    }
}
...
function basket_show() {
    $result_basket = sql_request("SELECT id_basket, pr_number,pr_name,gender,size,color,photo,price_unit,quantity FROM basket WHERE id_basket>0");
    $row = 1;
    $basket_list = '';
    $total_sum = 0;
